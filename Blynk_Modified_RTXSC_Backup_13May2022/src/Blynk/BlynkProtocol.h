 ...
 * @copyright  Copyright (c) 2015 Volodymyr Shymanskyy - 2022 RTXSC
 * @date       Jan 2015 / April 2022 RTXSC
...
public:
    ...
    // BLYNK_TIMEOUT_MS 6000UL from BlynkConfig.h
    bool connect(uint32_t timeout = BLYNK_TIMEOUT_MS*3) {
        ...
        {
            BLYNK_LOG2(BLYNK_F("[BLYNK PROTOCOL] CONTACTING BLYNK elapse: "), (BlynkMillis() - started)/1000);
            run();
            BlynkDelay(100);
        }
    ...
    if (hdr.type == BLYNK_CMD_RESPONSE) {
        ...
        if (state == CONNECTING && (1 == hdr.msg_id)) {
            ...
            case BLYNK_ALREADY_REGISTERED:
                BLYNK_LOG3(BLYNK_F("[BLYNK PROTOCOL] Ready (ping: "), lastActivityIn-lastHeartbeat, BLYNK_F("ms)."));
...
#endif
        // BLYNK_LOG1("[BLYNK PROTOCOL] LETS TRY TO CONNECT TO BLYNK!"); // this will get printed dozens of times
        return;
    }
